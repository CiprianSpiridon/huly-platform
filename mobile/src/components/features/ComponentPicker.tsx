/**
 * Component picker bottom sheet.
 *
 * Fetches available components for a project and lets the user pick one.
...
import { useComponents } from '@/hooks/useProjects'
...
interface ComponentPickerProps {
  ...
  onSelect: (componentId: Ref<Doc> | null) => void
}
...
const ComponentPicker = forwardRef<BottomSheetModal, ComponentPickerProps>(
  function ComponentPicker({ projectId, currentComponentId, onSelect }, ref) {
    ...
    const renderItem = useCallback(
      ({ item }: { item: ComponentItem }) => {
        const isSelected = item._id === currentComponentId
        return (
          <Pressable
            ...
            accessibilityLabel={`Select component: ${item.name}`}
      ...
      [currentComponentId, handleSelect]
    )

    const keyExtractor = useCallback(
      (item: ComponentItem) => item._id,
      []
    )
      ...
      >
        ...
        <Pressable
          className={`flex-row items-center gap-3 px-4 py-3 min-h-[44px] border-b border-border-primary ${
            currentComponentId == null ? 'bg-accent-subtle' : ''
          }`}
          onPress={() => handleSelect(null)}
          accessibilityRole="button"
          accessibilityLabel="No component"
          accessibilityState={{ selected: currentComponentId == null }}
        >
